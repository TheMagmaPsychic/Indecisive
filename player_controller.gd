...
func manual_physics_process(delta, original_delta):
	...
	if is_on_floor(): #If touching floor, allow movement (will replace with friction mechanics)
		...
		if Input.is_action_just_pressed("Jump"):
			jump()
	var normalized_speed = Vector3(velocity.x, 0, velocity.z).normalized()
	...
	if !is_on_floor():
		...
		var difference = normalized_speed - movement_dir_normal
		#if difference is above_min() and difference is below_max():
			#movement_speed *= 0.9
		#TODO make this work, slow down when input reverse of movement
...
func get_friction():
	...
	match len(floors):
		0:
			Global.output("Defaulting to 0.6", Global.urgencies.WARNING, print_friction)
			return(0.6)
		1:
