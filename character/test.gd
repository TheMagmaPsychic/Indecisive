...
var gravity = 15
#idle rotation
var xlocation = randi_range(-360,360)
var zlocation = randi_range(-360,360)
...
#map navigation
@onready var agent : NavigationAgent = $NavigationAgent3D
@onready var target_location = goingto

func _on_Timer_timeout():
 $Timer.set_wait_time(randf_range(1,3))
 xlocation = randf_range(-360,360) 
 zlocation = randf_range(-360,360)
 #random speed of idle move
 idle_speed = randf_range(minimum_speed, speed)
...
func _process(delta):
 if on_the_way == true:
  ...
  var target_pos = goingto.global_position
...
func _on_NavigationAgent_target_reached():
 on_the_way = false
 start_move = false
 $Timer2.set_wait_time(randf_range(4,8))
